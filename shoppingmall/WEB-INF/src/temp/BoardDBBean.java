...
public class BoardDBBean {
	...
	/*
	// MySQL
	static final String url = "jdbc:mysql://localhost:3306/shop?useUnicode=true&characterEncoding=euc-kr";
 ...
 */
 ...
 public int insertBoard(BoardBean board) throws Exception{
   ...
   try{ 
     ...
     sql="select max(b_id) from tboard"; 
     ...
     if(id!=0){  //답글이라면 부모글들을 찾아 그 중에서 현재글(원부모글)보다 
       ...
       sql="update tboard set b_step=b_step+1 where b_ref=? and b_step > ?";
       pstmt=con.prepareStatement(sql);
       pstmt.setInt(1,ref);
       pstmt.setInt(2,step);
       pstmt.executeUpdate();
     ...
     sql="insert into tboard(b_id, b_name, b_email, b_title, ";
 ...
 public ArrayList<BoardBean> listBoard(String pageNumber){  
   ...
   try {	     
     ...
     pageset = stmt.executeQuery("select count(b_id) from tboard");
	 ...
	 String str = "select * from  tboard order by b_ref desc, b_step asc";
 ...
 public BoardBean getBoard(int bid, boolean hitadd){
   ...
   try { 
	 ...
	 if(hitadd==true){
	 //조회수를 올린다
	 sql="update tboard set b_hit=b_hit+1 where b_id=?";
	 ...
	 sql="select * from tboard where b_id=?"; 
	 pstmt=con.prepareStatement(sql);
	 pstmt.setInt(1, bid);
	 rs = pstmt.executeQuery();  
	 if(rs.next()) { 
 ...
 public int editBoard(BoardBean board){
   ...
   try { 
	 ...
	 sql="select b_pwd from tboard where b_id=?"; 
	 pstmt=con.prepareStatement(sql);
	 ...
	 if(rs.next()) {
	   ...
	   if(!pwd.equals(board.getB_pwd()))
		 ...
		 sql="update tboard set b_name=?, b_email=?, b_title=?,";
   ...
   try { 
	 ...
	 if(rs.next()) {
	   pwd=rs.getString(1);	
		 ...
		 sql="delete from tboard where b_id=?";
...
//8)Admimd 으로 board 테이블의 내용을 삭제하는 메소드
public int deleteMBoard(int b_id, String mem_pwd){
  Connection con = null;   
  PreparedStatement pstmt=null;
  ResultSet rs=null;
  ...
  try { 
	 ...
	 sql="select mem_pwd from tmember where mem_uid=?"; 
