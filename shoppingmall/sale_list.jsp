...
<BODY>
	...
	<CENTER>
		<TABLE border=0 width=95% cellpadding=4 cellspacing=4
			...
			<TR>
				<TH width=5% bgcolor=skyblue><FONT color=black face=굴림><NOBR>번호</NOBR></FONT>
				</TH>
				<TH width=20% bgcolor=skyblue><FONT color=black face=굴림><NOBR>상품명</NOBR></FONT>
				</TH>
				<TH width=20% bgcolor=skyblue><FONT color=black face=굴림><NOBR>제조원</NOBR></FONT>
				</TH>
				<TH width=25% bgcolor=skyblue><FONT color=black face=굴림><NOBR>주문 수량</NOBR></FONT></TH>
				<TH width=10% bgcolor=skyblue><FONT color=black face=굴림><NOBR>판매가(원)</NOBR></FONT>
				</TH>
				<TH width=15% bgcolor=skyblue><FONT color=black face=굴림><NOBR>합계(수량*판매가)</NOBR></FONT>
				</TH>
			</TR>
			<%
				long id = 0;
				...
				try {
					...
					//con = DriverManager.getConnection(
						//	"jdbc:mysql://localhost:3306/shop?useUnicode=true&characterEncoding=euckr", "root", "1234");
				...
				try {
					...
					for (int i = 0; i < a.length; i++) {
						if( a[i].equals("uid") || a[i].equals("name") || a[i].equals("Member") )
							continue;
						...
						if (rs.next()) {
							...
							out.println("<INPUT type=text name=quantity size=2 value=" + qty + ">개 / 재고: " + stock + "개");							
