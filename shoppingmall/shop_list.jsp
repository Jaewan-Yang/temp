...
<SCRIPT language="javascript">
	function view(temp) {
		if (temp.length > 0) {
			url = "http://localhost:8080/shoppingmall/upload/" + temp;
			window
					.open(url, "win",
...
<BODY>
	...
	<FORM method=post name=search action="shop_list.jsp">
		<TABLE border=0 width=95%>
			...
			<TR>
				<TH><FONT size=-1> <A href="shop_list.jsp">전체</A>- <A href="shop_list.jsp?cat=11">가구</A>-
				...
				try {
					...
					if (!(rs.next()))
						...
						for (int j = startrow; j <= endrow; j++) {
							...
							out.println("재고: " + stock.elementAt(j) + "개&nbsp;&nbsp;");
							...
							if (stocki > 0) {
								...
								out.println("<INPUT type=hidden name=stock value=" + stock.elementAt(j) + ">");
